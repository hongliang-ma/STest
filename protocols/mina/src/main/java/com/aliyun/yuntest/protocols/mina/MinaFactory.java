...
public class MinaFactory {
	...
	public NioSocketAcceptor createAcceptor(
			ProtocolCodecFactory protocolCodecFactory, IoHandler handler,
			boolean isSecure) {
		...
		// measure the time it takes for a method in the IoFilterAdapter class
		// to execute.
		ProfilerTimerFilter profilerFilter = new ProfilerTimerFilter(
		...
		if (isSecure) {
			try {
				...
				throw new YunException("fail to add SSL support for server !");
			}
		}
		...
		if (handler != null) {
			...
			throw new YunException("no handler for server");
		}

		return acceptor;
	}

	public NioSocketConnector createConnector(IoHandler handler,
