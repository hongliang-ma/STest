package com.aliyun.yuntest.protocols.http;
...
public class AuthenticationHelper {
	...
	public static Object getAuthenticationResult(HttpContext context)
	...
	private final HttpRequestInterceptor m_reqInterceptor;

	private final HttpResponseInterceptor m_respInterceptor;
	...
	public AuthenticationHelper(Authenticator authenticator) {
		...
		m_reqInterceptor = new HttpRequestInterceptor() {
			...
			public void process(HttpRequest request, HttpContext context)
				...
				Object result = null;
				try {
					result = m_authenticator.authenticate(request
							.getFirstHeader("Authorization"));
					if (result != null) {
						context.setAttribute(
								CONTEXT_ATTR_AUTHENTICATION_RESULT, result);
					}
				} catch (AuthenticationException e) {
					m_logger.warn("fail to authenticate the HTTP request from host server: "
							+ e);
		...
		m_respInterceptor = new HttpResponseInterceptor() {
			...
			public void process(HttpResponse response, HttpContext context)
					throws HttpException, IOException {
				Object authResult = context
						.removeAttribute(CONTEXT_ATTR_AUTHENTICATION_RESULT);
