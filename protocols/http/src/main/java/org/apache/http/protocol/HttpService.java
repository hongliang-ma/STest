...
/**
 * HttpService is a server side HTTP protocol handler based in the blocking I/O model that implements the essential requirements of the HTTP protocol
 * for the server side message processing as described by RFC 2616. <br>
 * HttpService relies on {@link HttpProcessor} to generate mandatory protocol headers for all outgoing messages and apply common, cross-cutting
 * message transformations to all incoming and outgoing messages, whereas individual {@link HttpRequestHandler}s are expected to take care of
 * application specific content generation and processing. <br>
 * HttpService relies on {@link HttpRequestHandler} to resolve matching request handler for a particular request URI of an incoming HTTP request. <br>
 * HttpService can use optional {@link HttpExpectationVerifier} to ensure that incoming requests meet server's expectations.
 * 
 * @since 4.0
 */
public class HttpService {
    ...
    public void handleRequest(final HttpServerConnection conn, final HttpContext context) throws IOException, HttpException {
        ...
        YunSessionMessage sessionMessage = null;
        ...
        try {
            ...
            if (response == null) {
                ...
                sessionMessage = doService(request, response, context);
            }
        ...
        //TO set receipt to session message
        if (sessionMessage != null) {
            try {
                sessionMessage.addReceipt(new Object());
            }
            catch (InterruptedException e) {
                throw new YunException("sessionMessage adds receipt error!", e);
            }
        }
    }
    ...
    /**
     * The default implementation of this method attempts to resolve an {@link HttpRequestHandler} for the request URI of the given request and, if
     * found, executes its {@link HttpRequestHandler#handle(HttpRequest, HttpResponse, HttpContext)} method.
     * <p>
     ...
     *             in case of HTTP protocol violation or a processing problem.
     */
    protected YunSessionMessage doService(final HttpRequest request, final HttpResponse response, final HttpContext context) throws HttpException,
            IOException {
        ...
        if (handler != null) {
            return handler.handle(request, response, context);
        }
        else {
            response.setStatusCode(HttpStatus.SC_NOT_IMPLEMENTED);
            return null;
